...
data_dir = Path(home_dir, '.local/share/satnogs-tabulation-helper/')

try:
    TLE_DIR = config('SATNOGS_TLE_DIR', default=Path(data_dir, 'tles'))
    WATERFALL_IMAGES_DIR = config('SATNOGS_WATERFALL_IMAGES_DIR', default=Path(data_dir, 'waterfalls'))
    ARTIFACTS_DIR = config('SATNOGS_ARTIFACTS_DIR', default=Path(data_dir, 'artifacts'))
    DOPPLER_OBS_DIR = config('SATNOGS_DOPPLER_OBS_DIR', default=Path(data_dir, 'doppler_obs'))
    OBSERVATIONS_DIR = config('SATNOGS_OBSERVATIONS_DIR', default=Path(data_dir, 'observations'))
    SITES_TXT = config('SATNOGS_SITES_TXT', default=Path(data_dir, 'sites.txt'))
...
class strf_path:
    def init_paths():
        ...
        if not Path(data_dir, 'sites.txt').exists():
            Path(data_dir, 'sites.txt').touch()
